...
- total: 18
- percent: 56
- current: 已完成飞书通道首版数据链路与客户端页面，待补完整 run/gate 操作和文档
...
- [ ] T11 消息转任务：实现客户端“转为任务”，复用现有 `startCoordinatorRun` 和 active run 约束。 | depends_on: [T06, T08, T10]
...
- [ ] T15 单元测试：覆盖 channel store、channel service、orchestrator callbacks、publisher 本地同步、IPC handler。 | depends_on: [T03, T04, T06, T07, T08]
